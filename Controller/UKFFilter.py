...
class UnscentedKalmanFilter(object):
    ...
    def __init__(self, dim_x, dt, hx, fx, points,
    ...
    def update(self, z, R=None, UT=None, hx_args=()):
        ...
        if R is None:
            R = np.diag(np.tile(self.R,len(hx_args[0])))

        self.sigmas_h = zeros((self._num_sigmas, hx_args[0].size))
        for i in range(self._num_sigmas):
        ...
        Pxz = zeros((self._dim_x, hx_args[0].size))
        for i in range(self._num_sigmas):
